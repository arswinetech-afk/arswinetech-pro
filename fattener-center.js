...
(function () {
  ...
  function openTrialHealth(id) {
    ...
    batchDetailsModal({
      ...
      bid: t.batch_id,
      actions: `<button type="button" class="btn ghost" onclick="document.getElementById('fcHealthModal').remove();openTrialReport('${esc(t.id)}')">📄 Open printable client report</button>`
    });
  }
      ...
      careRow: _fost && Array.isArray(b.foster_from) && b.foster_from.length ? `
        <div class="foster-batch-card" style="background:linear-gradient(135deg,rgba(236,72,153,0.15),rgba(168,85,247,0.12));border:1.5px solid rgba(236,72,153,0.5);border-radius:12px;padding:12px 14px;margin-bottom:14px">
          ...
          <div style="font-size:12.5px">
            ${b.foster_from.map(f => `<div>⇢ <b>${f.males}♂ + ${f.females}♀</b> fostered from <b>${esc(f.from)}</b> (Dam: ${esc(f.dam || '—')}${f.breed ? ' · ' + esc(f.breed) : ''})</div>`).join('')}
          </div>
        </div>
      ` : '',
      actions: `<button type="button" class="btn" onclick="document.getElementById('fcHealthModal').remove();window.openBatchLedger && window.openBatchLedger('${esc(bid)}')">📊 Open batch hub</button>` +
