...
(function() {
  const realDays = date => date ? Math.max(0, Math.floor((new Date() - new Date(date + 'T00:00:00')) / 86400000)) : 0;
  ...
  function openSowTreatmentModal(index) {
    ...
    document.body.insertAdjacentHTML('beforeend', `<div class="due-modal-bg" id="sowTreatmentModal"><form class="due-modal" onsubmit="saveSowTreatment(event,${index})"><h2>Record Treatment: ${sow.name}</h2><div class="field"><label>Medicine Name</label><input name="medicine" required></div><div class="field"><label>ML Injected</label><input name="dosage_ml" type="number" step=".1" required></div><div class="field"><label>Date and Time</label><input name="date" type="datetime-local" value="${new Date().toISOString().slice(0,16)}" required></div><div class="field"><label><input name="follow" type="checkbox" onchange="document.getElementById('followDate').style.display=this.checked?'block':'none'"> Needs follow-up</label></div><div class="field" id="followDate" style="display:none"><label>Follow-up Date/Time</label><input name="follow_date" type="datetime-local"></div><div class="due-actions"><button type="button" class="btn ghost" onclick="document.getElementById('sowTreatmentModal').remove()">Cancel</button><button class="btn">Save Treatment</button></div></form></div>`)
  }
  ...
  function openDrilldown(kind) {
    ...
    document.body.insertAdjacentHTML('beforeend', `<div class="drill-bg" id="drillModal"><div class="drill-panel">
      ...
      <div class="drill-controls">
        <input class="search" placeholder="Search records" oninput="filterDrilldown(this.value)">
  ...
  function filterDrilldown(q) {
    ...
    document.querySelectorAll('#drillList .drill-sow,#drillList .drill-row').forEach(x => x.style.display = x.textContent.toLowerCase().includes(q) ? '' : 'none')
  }
