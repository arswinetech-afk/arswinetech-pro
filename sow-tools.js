...
(function() {
  function deleteRecord(kind, index) {
    let c = (typeof configs !== 'undefined' ? configs : window.configs)?.[kind] || { key: kind },
      list = F()[c.key],
      record = list[index];
    if (!record) return;
    if (!confirm(`Delete ${record.name||record.id||record.title||record.type||'this record'}? This cannot be undone.`)) return;
    ...
    const localId = String(record.id || record.no || record.tag || record.code || record.name || '').trim();

    // 1. Add to persistent tombstone list so sync never resurrects it
    F().deleted_ids = F().deleted_ids || [];
    if (localId && !F().deleted_ids.includes(localId)) {
      F().deleted_ids.push(localId);
    }
    if (record.id && !F().deleted_ids.includes(record.id)) {
      F().deleted_ids.push(record.id);
    }
    if (record.name && !F().deleted_ids.includes(record.name)) {
      F().deleted_ids.push(record.name);
    }

    // 2. Remove from all local DB buckets
    if (window.DB) {
      Object.keys(DB).forEach(fKey => {
        if (DB[fKey] && Array.isArray(DB[fKey][c.key])) {
          DB[fKey][c.key] = DB[fKey][c.key].filter(x => {
            const xKey = String(x.id || x.no || x.tag || x.code || x.name || '').trim();
            return xKey !== localId && x.id !== record.id && x.name !== record.name;
          });
        }
      });
    }
    ...
    if (window.ARSCloud && typeof ARSCloud.deleteAppRecord === 'function' && farmId) {
      if (localId) ARSCloud.deleteAppRecord(farmId, entityType, localId).catch(() => {});
      if (record.id && record.id !== localId) ARSCloud.deleteAppRecord(farmId, entityType, record.id).catch(() => {});
      if (record.name && record.name !== localId) ARSCloud.deleteAppRecord(farmId, entityType, record.name).catch(() => {});
    }
  ...
  function openSowProfile(index) {
    ...
    if (!sow) return;
    document.body.insertAdjacentHTML('beforeend', `<div class="due-modal-bg" id="sowProfileModal"><div class="due-modal sow-profile"><div class="modal-top"><div><div class="eyebrow">SOW PROFILE · FARM-SCOPED</div><h2>${sow.name}</h2><p>${sow.id} · ${sow.breed||'Breed not recorded'}</p></div><button class="close-reminder" onclick="closeSowProfile()">×</button></div><div class="sow-profile-grid"><div><small>Current status</small><b>${status(sow)}</b></div><div><small>Parity</small><b>${sow.parity||0}</b></div><div><small>Sire</small><b>${sow.sire||'Not recorded'}</b></div><div><small>Dam</small><b>${sow.dam||'Not recorded'}</b></div><div><small>Insemination</small><b>${fmtDate(sow.insemination)}</b></div><div><small>Gestation</small><b>${sow.insemination?days(sow.insemination)+' days':'—'}</b></div><div><small>Expected farrowing</small><b>${sow.insemination?fmtDate(isoOff(114-days(sow.insemination))):'—'}</b></div></div><div class="pedigree-mini"><b>Pedigree summary</b><p class="muted">Sire: ${sow.sire||'Unknown'} · Dam: ${sow.dam||'Unknown'}</p><small class="muted">Use Breed / Inseminate to run the farm’s relationship screening before recording semen.</small></div><div class="profile-actions"><button class="btn ghost" onclick="openHeatRecord(${index})">☼ Record heat</button><button class="btn" onclick="openBreedSow(${index})">⌁ Breed / inseminate</button>${sow.insemination && !sow.farrowingDate && !sow.lactationStart ? `<button class="btn ghost" onclick="closeSowProfile();openLinkedPigletModal(null,'${sow.id}')">🐷 Record farrowing</button>` : ''}<button class="btn ghost" onclick="editRecord('sows',${index});closeSowProfile()">Edit record</button><button class="btn danger-btn" onclick="deleteRecord('sows',${index});closeSowProfile()">Delete sow</button></div></div></div>`)
  }
  ...
  function saveHeatRecord(e, index, isReheat = false) {
    ...
    try {
      ...
      toast(Boolean(isReheat) ? 'Reheat saved. Breeding cycle marked failed.' : 'Heat record saved.');
      setTimeout(() => {
        try {
          renderAll();
