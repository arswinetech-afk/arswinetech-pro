...
(function () {
  ...
  function saveSemenSale(e, initialIdx) {
    e.preventDefault();
    let form = e.target,
    ...
    try {
      ...
      saleLines.forEach(l => {
        ...
        if (targetLot) {
          if (targetLot.available_bottles !== undefined) targetLot.available_bottles = Math.max(0, targetLot.available_bottles - l.qty);
          if (targetLot.bottles !== undefined) targetLot.bottles = Math.max(0, targetLot.bottles - l.qty);
        }
      });
      ...
      document.getElementById('semenSellModal')?.remove();
      renderAll();
      toast(`✔ Semen sale saved — ${peso(grandTotal)} to ${sale.customer}${paid > grandTotal ? ` · change ${peso(paid - grandTotal)}` : ''}`);
  ...
  function saveSemenAdjust(e, id) {
    ...
    try {
      ...
      adjState = null;
      renderAll();
      toast(`${kind === 'replacement' ? '🔁 Replacement' : '↩ Return'} logged — ${totalRet} back to stock${gives.length ? ` · ${gives.reduce((a, g) => a + g.qty, 0)} given` : ''} · collectibles realigned (${charge - credit > 0 ? '+' + peso(charge - credit) : charge - credit < 0 ? '−' + peso(credit - charge) : 'even'})`);
      document.getElementById('semenReceipt')?.remove();
  ...
  function saveSemenPayment(e, id) {
    e.preventDefault();
    ...
    try {
      ...
      document.getElementById('semenPayModal')?.remove();
      renderAll();
      toast(s.balance === 0 ? `✔ Fully settled — ${s.customer} paid the last ${peso(amt)}` : `✔ Payment ${peso(amt)} recorded — ${peso(s.balance)} still collectible from ${s.customer}`);
      document.getElementById('semenReceipt')?.remove();
  ...
  function saveSemenRestock(e) {
    ...
    try {
      ...
      toast(`✔ Restocked ${x.semen_batch_no || 'batch'} — now ${semenBottleCount(x)} bottle(s)`);
    } catch (ex) {
  ...
  function saveSemenNewBatch(e) {
    ...
    try {
      ...
      renderAll();
      if (window.refreshOpenDrilldown) refreshOpenDrilldown();
      toast(`🧪 New batch ${batchNo} saved — ${qty} bottle(s) on hand`);
    } catch (ex) {
  ...
  function openSemenResellerHub() {
    ...
    const totalBilled = txs.reduce((acc, tx) => acc + (+tx.total_amount || 0), 0);
    const totalCollected = txs.reduce((acc, tx) => acc + (+tx.paid_amount || 0), 0);
  ...
  function renderResellerAccountCardHTML(r, rIdx, allTxs) {
    const rTxs = allTxs.filter(tx => tx.reseller_id === r.id || tx.reseller_name === r.name);
    const rBilled = rTxs.reduce((acc, tx) => acc + (+tx.total_amount || 0), 0);
    const rPaid = rTxs.reduce((acc, tx) => acc + (+tx.paid_amount || 0), 0);
    const rBal = Math.max(0, rBilled - rPaid);
      ...
      <div class="reseller-tx-row" id="txRow_${tx.id}">
        <div class="reseller-tx-top">
          ...
          <div style="text-align:right">
            ...
            <small class="muted" style="display:block;font-size:11px">Paid: <b>${peso(tx.paid_amount || 0)}</b> · Bal: <b style="color:${tx.balance > 0 ? 'var(--warn)' : 'var(--ok)'}">${peso(tx.balance || 0)}</b></small>
          </div>
        </div>
  ...
  function deleteResellerProfile(resellerId) {
    ...
    const rBal = rTxs.reduce((acc, tx) => acc + (+tx.total_amount || 0) - (+tx.paid_amount || 0), 0);
        ...
        <form class="due-modal reseller-hub-wrap" onsubmit="window.saveResellerPickup(event)">
          ...
          <div class="reminder-fields" style="text-align:left">
            ...
            <div class="field full">
              ...
              <input type="datetime-local" id="resellerPickupTime" name="timestamp" value="${new Date().toISOString().slice(0, 16)}" required class="suggest-input">
            </div>
  ...
  window.saveResellerPickup = function(e) {
    if (e) e.preventDefault();
    const f = F();
    const form = e.target;
    const d = Object.fromEntries(new FormData(form));
    ...
    validLines.forEach(l => {
      ...
      if (s) {
        const remaining = Math.max(0, +(s.available_bottles !== undefined ? s.available_bottles : (s.bottles || 0)) - (+l.qty || 0));
    ...
    document.getElementById('resellerPickupModal')?.remove();
    toast(`✔ Pickup #${txId} recorded for ${reseller.name}!`);
    openSemenResellerHub();
  ...
  function openResellerPaymentModal(resellerId, txId = '') {
    ...
    const rTxs = (f.semenResellerTx || []).filter(tx => tx.reseller_id === r.id);
    const rBilled = rTxs.reduce((acc, tx) => acc + (+tx.total_amount || 0), 0);
    const rPaid = rTxs.reduce((acc, tx) => acc + (+tx.paid_amount || 0), 0);
    const rBal = Math.max(0, rBilled - rPaid);
        ...
        <form class="due-modal" style="text-align:left" onsubmit="window.saveResellerPayment(event, '${r.id}', '${txId || ''}')">
          ...
          <div class="reminder-fields">
            <div class="field full">
              ...
              <input type="number" min="1" step="1" name="amount" value="${rBal || ''}" required placeholder="e.g. 1500" class="suggest-input" style="font-size:18px;font-weight:bold;color:var(--ok)">
            </div>
            <div class="field">
              <label>Payment Date &amp; Time *</label>
              <input type="datetime-local" name="timestamp" value="${new Date().toISOString().slice(0, 16)}" required class="suggest-input">
            </div>
            <div class="field">
              <label>Payment Method *</label>
          ...
          <div class="due-actions" style="margin-top:16px">
            ...
            <button type="submit" class="btn" style="background:var(--ok);color:#fff">✓ Record Payment &amp; Print BLE</button>
          </div>
        </form>
      </div>
    `);
  ...
  window.saveResellerPayment = function(e, resellerId, txId) {
    ...
    if (payAmt <= 0) { toast('Please enter a valid payment amount.'); return; }

    // Allocate payment against transactions
    let remainingPay = payAmt;
    const rTxs = (f.semenResellerTx || []).filter(tx => tx.reseller_id === r.id && tx.balance > 0);

    // If specific tx was chosen, pay it first
    if (txId) {
      const specificTx = rTxs.find(tx => tx.id === txId);
      if (specificTx) {
        const credit = Math.min(specificTx.balance, remainingPay);
        specificTx.paid_amount = (specificTx.paid_amount || 0) + credit;
        specificTx.balance = Math.max(0, specificTx.total_amount - specificTx.paid_amount);
        remainingPay -= credit;
      }
    }

    // Allocate remaining to oldest unpaid transactions
    for (const tx of rTxs) {
      if (remainingPay <= 0) break;
      if (tx.balance > 0) {
        const credit = Math.min(tx.balance, remainingPay);
        tx.paid_amount = (tx.paid_amount || 0) + credit;
        tx.balance = Math.max(0, tx.total_amount - tx.paid_amount);
        remainingPay -= credit;
      }
    }

    // Log farm income transaction
    (f.transactions = f.transactions || []).unshift({
      date: (d.timestamp || new Date().toISOString()).slice(0, 10),
      type: 'Income',
      category: 'Semen Sales',
    ...
    save();
    document.getElementById('resellerPayModal')?.remove();
    toast(`✔ Payment of ₱${payAmt.toLocaleString()} recorded for ${r.name}!`);
    openSemenResellerHub();

    // Print statement / payment receipt
    window.btPrintResellerStatement(r.id, payAmt);
  };
        ...
        <form class="due-modal" style="text-align:left" onsubmit="window.saveEditResellerTx(event, '${tx.id}')">
          ...
          <div class="reminder-fields">
            <div class="field full">
              ...
              <input type="datetime-local" name="timestamp" value="${tx.timestamp ? new Date(tx.timestamp).toISOString().slice(0,16) : new Date().toISOString().slice(0,16)}" required class="suggest-input">
            </div>
            <div class="field">
        ...
        <div class="semen-receipt-wrap">
          <div class="sale-receipt">
            ...
            <div class="rc-totals">
              ...
              <div style="display:flex;justify-content:space-between"><span>BALANCE DUE:</span><b style="color:${tx.balance > 0 ? 'var(--warn)' : 'var(--ok)'}">${peso(tx.balance || 0)}</b></div>
            </div>
          ...
          <div class="sale-receipt">
            ...
            ${rTxs.map(tx => `
              <div style="font-size:11.5px;padding:4px 0;border-bottom:1px dashed var(--line)">
                ...
                <small class="muted">Paid: ${peso(tx.paid_amount || 0)} · Bal: ${peso(tx.balance || 0)}</small>
              </div>
  ...
  async function btPrintResellerTx(txId) {
    ...
    try {
      ...
      add(row("BALANCE DUE:", P(tx.balance || 0)));
  ...
  async function btPrintResellerStatement(resellerId, justPaid = 0) {
    ensureResellerData();
    const f = F();
    const r = (f.semenResellers || []).find(x => x.id === resellerId);
    if (!r) return;
    ...
    try {
      ...
      rTxs.slice(0, 6).forEach(tx => {
        ...
        add(row(`  Billed ${P(tx.total_amount)}`, `Bal ${P(tx.balance)}`));
      });
