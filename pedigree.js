...
(function() {
  ...
  function buildAnimalNode(rawRef, side = 'root', gen = 0, relLabel = 'Subject Animal') {
    ...
    const resolved = window.resolveAnimalLabel ? window.resolveAnimalLabel(rawRef) : { name: String(rawRef), id: String(rawRef), breed: '' };
    const hit = resolved.hit || (typeof byId === 'function' ? byId(rawRef) : null);

    let sireRef = hit ? (hit.sire || hit.sireRef || hit.sire_name || hit.sire_id || '') : '';
    let damRef = hit ? (hit.dam || hit.damRef || hit.dam_name || hit.dam_id || '') : '';
    ...
    return {
      ...
      kind: hit ? (hit.kind || (hit.sex === 'M' ? 'Boar' : (hit.sex === 'F' ? 'Sow' : 'Ancestor'))) : 'Ancestor',
      sex: (hit && hit.sex) ? hit.sex : (side.includes('sire') || side === 'sire' ? 'M' : 'F'),
      gen,
      relLabel,
      hit,
      exists: !!hit || (resolved.name && resolved.name !== '—' && String(resolved.name).toLowerCase() !== 'unknown'),
      ...
      <div class="tree-hierarchy-layout">
        ...
        <div class="tree-col col-subject">
          <div class="tree-card root-card active" id="node_root" onclick="window.selectPedNode('root')">
            <div class="tc-badge" style="color:var(--teal2)">👑 SUBJECT ANIMAL</div>
            <div class="tc-name">${escP(root.name)}</div>
            <div class="tc-sub">${escP(root.breed || 'Breed —')} · ${root.sex === 'M' ? '♂ Male' : '♀ Female'}</div>
            <div class="tc-gen">100% Target Herd Alleles</div>
          </div>
        </div>
  ...
  function selectPedNode(nodeKey) {
    ...
    let target = root;
    let pct = '100% Target Animal';
    let rel = 'Subject Herd Animal';
    ...
    if (titleEl) {
      titleEl.innerHTML = `<b>${target.sex === 'M' ? '♂' : '♀'} ${escP(target.name)}</b> <span class="tag" style="font-size:11px;margin-left:6px">${escP(rel)}</span>`;
    }
  ...
  function openPedigreeTreeModal(idOrAnimal) {
    ...
    const treeRoot = buildAnimalNode(a.id || a.name, 'root', 0, 'Subject Animal');
    ...
    document.body.insertAdjacentHTML('beforeend', `
      <div class="due-modal-bg" id="pedigreeTreeModal" style="z-index:999999!important;position:fixed!important;inset:0!important;display:grid!important;place-items:center!important">
        <div class="due-modal pedigree-tree-modal">
          <div class="modal-top">
            <div>
              <div class="eyebrow" style="color:var(--teal2);font-weight:700">🧬 3-GENERATION GENETIC PEDIGREE &amp; ANCESTRY TREE</div>
              <h2>${escP(a.name || a.id)} <span class="tag" style="background:#059669;color:#fff">${escP(a.breed || 'Pedigree Animal')} · ${a.parity ? 'P' + a.parity : (a.kind || 'Breeder')}</span></h2>
              <p class="perf-sub">Interactive Ancestry Visualizer · Tap any ancestor node to trace genetics &amp; relationships</p>
            </div>
