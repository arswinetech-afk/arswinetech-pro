...
(function () {
  ...
  function armScheduleReminder(s) {
    ...
    const r = {
      ...
      title: '💉 Vaccination due: ' + s.vaccine + ' — ' + s.target_label,
      description: 'Scheduled vaccination on ' + fmtD(s.date) + ' at ' + (s.time || '08:00') + (s.note ? ' · ' + s.note : '') + ' — open the Vaccination center and tap 💉 Record now.',
  ...
  function vaxPage() {
    ...
    const schedRows = scheds.filter(schedOk).map(x => {
      ...
      return `<div class="vax-row sched" data-vax>
        <span class="vax-ico">${TYPE_META[x.target_type].ico}</span>
        <div class="vax-main"><b>${esc(x.vaccine)}</b>
          <small>${esc(x.target_label)} · ${TYPE_META[x.target_type].label}</small>
          <small>first dose scheduled${x.note ? ' · ' + esc(x.note) : ''}</small>
        </div>
        ...
        <div class="vax-cta"><button class="btn" onclick="vaxRecordFor('${x.target_type}',${jsq(x.target_id)},${jsq(x.target_label)},${jsq(x.id)})">💉 Record now</button><button class="btn ghost" onclick="openVaxSchedule('${x.target_type}',${jsq(x.target_id)},${jsq(x.target_label)})" title="Edit schedule">✏️</button><button class="btn ghost delete-action" onclick="cancelVaxSchedule(${jsq(x.id)})" title="Cancel this schedule">🗑</button></div>
      </div>`;
    }).join('');
  ...
  function cancelVaxSchedule(id) {
    ...
    save();
    renderAll();
    toast('Vaccination schedule cancelled');
  }
  ...
  function openVaxReport() {
    ...
    const schedTable = schedPend.length
      ? `<table class="vax-rep">${tr(['Animal / group', 'Vaccine', 'Date', 'Time', 'Alarm'], true)}${schedPend.map(x => tr([`${TYPE_META[x.target_type].ico} <b>${esc(x.target_label)}</b><br><small>${TYPE_META[x.target_type].label}</small>`, esc(x.vaccine) + (x.note ? `<br><small>${esc(x.note)}</small>` : ''), `<b>${fmtD(x.date)}</b>`, esc(x.time || '08:00'), '🔔 armed'])).join('')}</table>`
