...
(function() {
  const allocationAvailable = (b, source, g) => {
    let l = F().pigletLedger || [],
      assigned = l.filter(t => t.batch_id === b.id && t.gender === g && t.type === source && !['undone', 'deleted'].includes(t.status)).reduce((a, t) => a + (+t.quantity || 0), 0),
      reserved = l.filter(t => t.batch_id === b.id && t.gender === g && t.type === 'reserved' && t.source === source && !['undone', 'deleted'].includes(t.status)).reduce((a, t) => a + (+t.quantity || 0), 0),
      cancelled = l.filter(t => t.batch_id === b.id && t.gender === g && t.type === 'cancel_reservation' && t.source === source && !['undone', 'deleted'].includes(t.status)).reduce((a, t) => a + (+t.quantity || 0), 0);
  ...
  const reservationRowHTML = (r, origIndex) => {
    ...
    return `
      <tr class="reservation-row ${isFloating ? 'floating-row' : ''}" style="cursor:pointer" onclick="window.openReservationDetails(${origIndex})" title="Tap to view / print reservation certificate">
        ...
        <td><b>${esc(r.batch_id)}</b>${resLineExtra(r)}${ageBadge}</td>
  ...
  function saveReservation(e) {
    ...
    try {
      ...
      page();
      renderAll();
      toast(isFloating ? `⏳ Floating priority reservation saved for ${r.customer}! (Prepayment: ${peso(paid)})` : (batchCount > 1 ? `Reservation saved · ${r.quantity} heads from ${batchCount} batches` : 'Reservation saved'));
    } catch (ex) {
      if (err) {
  ...
  function allocateFloatingSlot(origIndex) {
    ...
    page();
    renderAll();
    toast(`🎉 Slot successfully allocated to ${r.customer} in Batch ${b.id}!`);
  }
  ...
  function reservationAction(i, status) {
  ...
  function saveRelease(e, i) {
    ...
    page();
    renderAll();
    toast(`🎉 Successfully released ${qty} piglet${qty > 1 ? 's' : ''} for ${r.customer}!`);
  }
  ...
  function saveReservationEdit(e, i) {
    e.preventDefault();
    ...
    page();
    renderAll();
    toast(`✓ Reservation for ${r.customer} updated successfully!`);
  }
