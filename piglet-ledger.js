...
(function() {
  ...
  function counts(b) {
    let l = (F().pigletLedger || []).filter(x => x.batch_id === b.id && !["undone", "deleted"].includes(x.status)),
  ...
  function saveAllocation(e, id, type) {
    e.preventDefault();
    let d = Object.fromEntries(new FormData(e.target)),
      b = batch(id),
      ok = true;
    if (+d.male) ok = ledger(type, b, 'male', +d.male, d.notes) && ok;
    if (+d.female) ok = ledger(type, b, 'female', +d.female, d.notes) && ok;
    if (ok) {
  ...
  function saveEditBatchTransaction(e, id) {
    e.preventDefault();
    const form = e.target;
    ...
    document.getElementById('editBatchTxModal')?.remove();
    refreshBatch(b.id);
    toast(`✔ Updated transaction: ${newQty} ${newGender} assigned to ${newType.replace('_', ' ')}`);
  }
  window.saveEditBatchTransaction = saveEditBatchTransaction;
  ...
  function undoBatchTransaction(id) {
    ...
    if (!confirm(`Undo this transaction (${x.quantity} ${x.gender} ${x.type.replace('_', ' ')})?

This will remove it from the transaction history and return the piglets back to the available pool.`)) return;

    // If it was a mortality with mirrored loss, remove from transactions
    if (x.type === 'mortality') {
      F().transactions = (F().transactions || []).filter(t => !t.description || !t.description.includes(x.batch_id) || !t.category.includes('Mortality'));
    }
  ...
  function deleteBatchTransaction(id) {
    ...
    if (!confirm(`Permanently delete this transaction (${x.quantity} ${x.gender} ${x.type.replace('_', ' ')})?

This will erase the record completely and restore the piglets to the available pool.`)) return;

    // Erase linked reservation if applicable
    if (x.type === 'reserved') {
      F().reservations = (F().reservations || []).filter(r => !(r.batch_id === x.batch_id && r.quantity === x.quantity));
    }

    // Erase linked mortality expense if applicable
    if (x.type === 'mortality') {
      F().transactions = (F().transactions || []).filter(t => !t.description || !t.description.includes(x.batch_id) || !t.category.includes('Mortality'));
    }

    // Remove completely from ledger
    list.splice(idx, 1);
    save();
  ...
  function pigletRowHTML(x) {
    ...
    return `<div class="drill-row batch-drill-row drill-row-link piglet-live-row ${x.archived ? 'drill-archived' : ''}" role="button" data-piglet-search="${hay}" onclick="openBatchDetails('${jid}')">` +
